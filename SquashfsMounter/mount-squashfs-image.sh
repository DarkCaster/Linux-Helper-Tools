...
test "z$check" = "z1" && log "image at $image already connected!" && exit 1

#check link validity
if [ "z$target" != "z" ]; then
	test -e "$target" && log "error: $target is not a symlink, or this symlink point to already mounted image!" && exit 1
...
test "$?" != "0" && log "last operation failed with output: $udisks_output" && exit 1

device=`echo "$udisks_output" | grep -oE '( )(\/dev\/loop)([0-9]*)' | tr -d '[:blank:]'`
test "$?" != "0" && log "device detection regexp failed!" && exit 1
test ! -b "$device" && log "error: device $device is not a /dev/loop block device" && exit 1
...
if [ "z$automount" != "ztrue" ]; then
 ...
 test "$?" != "0" && log "mount failed!" && exit 1
fi
...
test "$mounted" = "false" && log "timed out while waiting for $device to mount" && exit 1
...
if [ "z$automount" != "ztrue" ]; then
 ...
 test "$?" != "0" && log "command failed!" && exit 1
fi
...
test ! -d "$mountpoint" && log "error: mountpoint $mountpoint directory is not exist!" && exit 1

ln -s "$mountpoint" "$target"
test "$?" != "0" && log "failed to create symlink to $mountpoint at $target" && exit 1
